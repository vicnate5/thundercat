...
================================================================================
       ...
       Building and Debugging Apache Thundercat under NetBeans
...
is also its weakness when working with complex projects such as Thundercat that
...
1. NetBeans can open a Thundercat source tree as a Free-Form Project, which
   will allow you to edit, build, and debug Thundercat and its unit tests
   within the workbench. Even with NetBeans 7.1, integration of a project
   as complex as Thundercat requires significant configuration. The
   configuration involves dealing with several quirky aspects of the way
   NetBeans manages this kind of project. Before you try to open Thundercat
   ...
   (see BUILDING.txt in the Thundercat source root directory).

2. Once Thundercat has been built, you can install the default NetBeans
   project configuration by running the following build target:

          ant ide-netbeans

   This uses the Thundercat build.xml to create a new directory called
   ...
   sufficient to open Thundercat as a Free-Form Project under NetBeans.
   ...
   restore the Thundercat default files at any time by running this target:
...
3. NetBeans needs to know where to find the directory where you keep the
   Thundercat dependencies jars. If you have successfully built Thundercat from
   a command prompt, you will probably have assigned the base.path
   property in your build.properties file.

   Warning: The support for Thundercat in NetBeans will detect and use this
   ...
   Note: The current support for Thundercat in NetBeans does not include the
...
4. Start NetBeans... once it has initialised and scanned your other open
   ...
   Thundercat. NetBeans will recognise it as a Free-Form project, then read and
   ...
   Warning: do not be tempted to casually click the properties menu item
            for the Thundercat project. NetBeans might change the contents
            of these files. (The NetBeans New Project wizard also
            automatically creates a Free-Form project.xml which carries
            this same warning).

   Note: the Thundercat project should open successfully and, after the source
...
5. Verify your work by running the NetBeans project Clean action. It
   ...
   the Thundercat deploy build target) and confirm that it successfully
   compiles the Thundercat source files and creates the jars.
...
6. Next, navigate down to one of the test files and select the compile
   ...
   Note: if you have changed any of the Thundercat source files, they will be
...
9. You can also use your Thundercat NetBeans Free-Form project to debug an
   external Thundercat instance that is executing on the same, or a different
   machine. (Obviously, the external instance must be running the same
   version of the source code!)

   The external Thundercat instance must be started with its jvm enabled for
   debugging by adding extra arguments to JAVA_OPTS, e.g.
   -Xdebug -Xrunjdwp:transport=dt_socket,address=8000,server=y,suspend=n

   To debug the external Thundercat instance under NetBeans, select the
   ...
   transport. Specify the hostname and port where the Thundercat jvm is
   listening. Your NetBeans workbench should then connect to the external
   Thundercat and display the running threads. You can now set breakpoints and
   begin debugging.
